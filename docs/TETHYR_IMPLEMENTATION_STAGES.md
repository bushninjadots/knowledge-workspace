...
- **Not verified, and recorded as such:** the _built_ server cannot be run locally. Both `npx wrangler dev -c .output/server/wrangler.json` and `node .output/server/index.mjs` (node-server preset) die with `TypeError: __exportAll is not a function` — a Nitro `_libs` chunk-splitting bug that predates this pass and is unrelated to it (`.output` is gitignored and the platform serves `npm run dev`). So the production header mode is asserted structurally (unit tests for the policy variants and for header preservation) plus behaviourally in dev under the report-only strict policy, and the hosted runtime (Workers, Sentry Replay, the Studio preview iframe) still needs a human check on deploy. Noted in `AGENTS.md` so the next person does not spend an afternoon on it.
