...
// After
const data = await supabase.from("projects").select("id, title, description, status, cover_url, created_at") as ProjectRow;
```
...
*Plan generated from the 2026-08-20 full site audit (78 findings). Groups fixes by batch for parallel subagent execution.*
