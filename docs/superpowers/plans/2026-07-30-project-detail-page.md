              ...
              >
                <img
                  ...
                  onError={(e) => {
                    ...
                    (e.target as HTMLImageElement).parentElement!.textContent = (creator.profile
...
export function ProjectMainContent({
  ...
  return (
    <div className="space-y-20">
        ...
        {contributors.length === 0 ? (
          ...
          <div className="space-y-3">
            {contributors.map((c) => (
              <div
                key={c.profile_id}
                className="flex items-center gap-3 rounded-xl bg-surface-elevated/40 p-3"
              >
                <Link
                  to="/u/$handle"
      ...
      {/* Gallery */}
      {((project.gallery ?? []) as { url: string; caption?: string; type: "image" | "video" }[])
        .length > 0 && (
