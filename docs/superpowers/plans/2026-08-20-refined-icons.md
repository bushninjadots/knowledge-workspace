# Refined Line Icons Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace 24 generic Lucide achievement icons with custom Tethyr-branded line icons, refine pillar icons, and add proper favicon + PWA manifest.

**Architecture:** Create `src/components/tethyr/icons/achievements.tsx` with 24 custom SVG components and a lookup map. Update `achievements.tsx` to import from the new module instead of Lucide. Refine pillar icons in `icons-system.tsx`. Add favicon SVG, PWA manifest, and link tags to root layout.

**Tech Stack:** React 19, inline SVG, TanStack Router (head links), no new dependencies.

## Global Constraints

- 1.5px stroke weight, 24×24 viewBox (badges), 20×20 (compact/nav)
- No fills — stroke-only, matching Lucide aesthetic
- `fill="none"` on all SVGs, `stroke="currentColor"` on paths
- `strokeLinecap="round" strokeLinejoin="round"` on all strokes
- Accept `className` prop via `SVGProps<SVGSVGElement>`
- Brand colors: `#7CFF6E`→`#5FE64A` (green), `#A64DFF`→`#8A2BFF` (purple)
...
```bash
mkdir -p src/components/tethyr/icons
```

- [ ] **Step 2: Write all 24 icon components**

Create `src/components/tethyr/icons/achievements.tsx` with:
...
```bash
git add src/components/tethyr/icons/achievements.tsx
git commit -m "feat: add 24 custom achievement badge icons"
```
...
- [ ] **Step 1: Update `AchievementDef` type in reputation.ts**

Change the `icon` field from `string` (Lucide name) to `string` (achievement type key):

```tsx
// src/lib/reputation.ts:50-56
...
No change needed — the field is already `string`. The values in `ACHIEVEMENTS` array already match the achievement type keys (e.g., `icon: "Rocket"` for `first_project`). We need to change these to match the type keys.

- [ ] **Step 2: Update icon values in ACHIEVEMENTS array**

In `src/lib/reputation.ts`, change each `icon` value from the Lucide name to the achievement type:

```tsx
// Example for first entry:
{ type: "first_project", ..., icon: "first_project", ... }
// Repeat for all 24 entries — icon value = type value
```

- [ ] **Step 3: Update achievements.tsx to use custom icons**

Replace the Lucide imports and `ICONS` map:

```tsx
// src/components/tethyr/achievements.tsx
// REMOVE these Lucide imports (lines 4-27):
// import { Award, Rocket, Flag, ... } from "lucide-react";

// ADD:
import { ACHIEVEMENT_ICONS } from "./icons/achievements";
import { Award } from "lucide-react"; // keep Award for fallback only

// REPLACE the ICONS map (lines 36-59) with:
// (removed entirely — ACHIEVEMENT_ICONS is used directly)
...
```bash
git add src/components/tethyr/achievements.tsx src/lib/reputation.ts
git commit -m "feat: wire custom achievement icons into badge rendering"
```
...
- Tighter proportions
- Same gradient on full-size, stroke-only on compact

Keep the existing component signatures and export names.

- [ ] **Step 2: Update compact variants to stroke-only**

Change compact variants from `fill="currentColor"` to stroke-based with `fill="none"`, `stroke="currentColor"`, `strokeWidth="1.5"`.
...
```bash
git add src/components/tethyr/icons-system.tsx
git commit -m "feat: refine pillar icon geometry and stroke consistency"
```
...
Create `public/favicon.svg` — simplified infinity mark (no text, no animation, optimized for 16px):

```svg
<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#7CFF6E"/>
      <stop offset="100%" stop-color="#8A2BFF"/>
    </linearGradient>
  </defs>
  <path d="M30 60C30 46.7 39.1 36 50 36C62.7 36 72 45.4 72 60C72 74.6 62.7 84 50 84C39.1 84 30 73.3 30 60Z" fill="url(#g)"/>
  <path d="M48 60C48 46.7 57.1 36 68 36C80.7 36 90 45.4 90 60C90 74.6 80.7 84 68 84C57.1 84 48 73.3 48 60Z" fill="url(#g)"/>
</svg>
```
...
```json
{
  "name": "Tethyr",
  "short_name": "Tethyr",
  "description": "Connected by what you know",
  "start_url": "/",
  "display": "standalone",
  "theme_color": "#0a0a0a",
...
- [ ] **Step 3: Add link tags to root layout**

In `src/routes/__root.tsx`, add to the `links` array (after line 121):
...
```bash
git add public/favicon.svg public/site.webmanifest src/routes/__root.tsx
git commit -m "feat: add SVG favicon and PWA manifest"
```
