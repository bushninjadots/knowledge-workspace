...
| --- | ------------------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 11  | No email verification enforcement visible — signup may allow unverified email access | Low-Med  | open — verification UX shipped (2026-09-07: unverified-email banner across authenticated pages + "resend confirmation" on login when blocked with `email_not_confirmed`); server-side enforcement (blocking unconfirmed logins / demo exemptions) is a Supabase auth config decision |
...
(edit mode, drag-drop, theme tokens), milestones, activity feed, discussions, open roles
...
| 2   | Client-side achievement checking — race risk; should be DB-only                      | Medium   | **fixed** (migration `20260905150000_event_driven_achievements.sql`: `trg_award_earned_achievements` AFTER INSERT on projects/comments/posts/project_contributors/skill_endorsements/teams/team_members + project_role_applications accepted; `award_earned_achievements(p_profile_id uuid DEFAULT NULL)`; dashboard client call kept as backstop) |     | 3   | Dual rendering path (blocks + legacy) — maintenance debt | Medium | **fixed** (2026-09-07) — the interactive workspace is now the only project render on `projects.$id`; the `blocksArePage` takeover, the orphaned Files/Activity tab bar in block mode, and the read-only block-page mount are removed. Blocks stay available through Studio/PageShell (profile-first) but no longer replace the workspace on projects |
...
| 7   | Banner caption/position changes on the dashboard did not reach the Studio — header block kept its own query and never got invalidated, and it hardcoded the default caption position/overlay                                                                                                                                                                                                                                | Medium   | **fixed** (2026-09-18) — `BannerStrip` and `BackgroundPickerDialog` writes now invalidate `profile-header-block` alongside `current-user` (same pattern as `HeroEditControls`), and the header block passes the profile's `bannerOverlay`/`bannerCaptionPosition` through instead of assuming defaults. Regression coverage: `banner-sync.test.tsx` (2 tests assert both writers invalidate both keys)                                                                                                                                                                                                                                                                                                                              |
| 8   | Public Studio (`/u/:handle`) didn't render like the owner's Studio view (`/profile`): blocks lacked the `studio-block` frame (background/border/radius/inset), template sections auto-flowed instead of using span boxes, the canvas painted opaque `bg-background` over the member's backdrop, the noise texture and `--studio-*`/font-hint variables were owner-view-only, and the theme applied without scheme awareness | Medium   | **fixed** (2026-09-21) — `PageLayoutRenderer` view mode now wraps blocks in the same `studio-block`/`studio-block-flush` frame as the owner view and sizes no-grid sections with span boxes; `PageShell` takes an optional `backgroundSlot` (public route paints the member's backdrop inside the themed canvas, which is now `bg-background bg-noise`) and applies the shared `studioSurfaceStyle`; the duplicated owner-view/editor `studioSurfaceStyle` helpers were extracted into `studio-config.ts` (single source; font constants de-exported); owner banner strip on own public page removed. Coverage: `page-layout.test.tsx` (5 parity tests) + `scripts/qa-studio-parity.mjs` (owner vs public frame census, nightly CI) |     | 9   | Profile picture had no shape choice — the avatar was hard-coded `rounded-full` everywhere and the appearance dialog (shared by Studio and Settings) had no silhouette control | Minor | **fixed** (2026-09-21) — `profiles.background.avatarShape` (new `AvatarShapeId`: circle, rounded, squircle, hex, diamond, flower/bloom, star) resolves via `normalizeAvatarShape` + `avatarShapeStyle()` into `--avatar-radius` / `--avatar-clip` CSS variables; `ui/avatar` consumes the variables (call sites opt in by scoping the vars, so unrelated lists keep the Tethyr circle); scope applied on the StudioView root, the public Studio shell, and the BasicProfile fallback; picker added to `BackgroundPickerDialog` (Studio "Change appearance" + Settings → Appearance, same editor). Follow-ups (same day): **avatar ring** (`avatarRing`/`avatarRingColor` → `--avatar-ring`, none/accent/custom picker, replaces the static surface ring on the Studio identity header + BasicProfile) and **team-block contributors** now render with each contributor's own shape/ring via a jsonb embed (no new query). Coverage: shape+ring tests in `background-themes.test.ts`, silhouette-aware crop-preview tests, + `scripts/qa-avatar-shape.mjs` (both Studios render shape and ring, reset restores defaults, nightly CI) |
...
| #   | Issue                                                                                                                             | Severity | Status                                                                                                                                                                                                                          |
| --- | --------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | 7 realtime channels, no cleanup/connection-pool audit (connections hook uses module-level singleton; others useRef per-component) | Low-Med  | **partially fixed** (2026-09-08) — every inspected owner removes its channel on unmount; duplicate `space-posts-*` ownership was consolidated into `useSpacePostsRealtime`; a full connection-pool abstraction remains deferred |
| 8   | Community search bar only filters current feed, not global search                                                                 | Low      | open — scoped to the community page's feed filter; note: a site-wide `GlobalSearch` (people/skills/projects/library/posts/sessions, dashboard sidebar + Cmd+K) already exists and ships on the `/` search box                   |
...
| # | Issue | Severity | Status |
| --- | ------------------------------------------------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- || 9 | No calendar integration (Google Calendar / iCal export) despite a "calendar" tab | Low | **fixed** (2026-09-16) — client-side RFC 5545 iCal export (`src/lib/ical.ts`, `session-ical.ts`); "Add to calendar" on the session detail page and on Upcoming/Today session list cards; 7 unit tests; browser-verified |
...
| #   | Issue                                                                                     | Severity | Status                                                                                                                           |
| --- | ----------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 17  | Skill matching is client-side — won't scale (needs server-side query / materialized view) | Low      | **fixed** (2026-09-08) — `match_projects` and `match_creators` RPCs score candidates in Postgres and only return bounded matches |
...
| 26  | Saved searches only for opportunities (localStorage), not projects/people                  | Low      | **fixed** (2026-09-16) — explore persists search/category per tab (Projects + People parity with Opportunities), restored on tab switch; browser-verified        |
...
| #   | Issue                                                                                                                                                                 | Severity | Status                                                                                                                                                                                                                                                                                                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 31  | N+1 signed URL generation for contributor avatars (`Promise.all` over avatarTargets)                                                                                  | Low-Med  | **fixed** (2026-09-05) — batched `createSignedUrls` in `projects.$id.tsx` and `team-block.tsx`; the remaining project-media cover loops in `explore.tsx` + `use-current-user.ts` were also batched into single `createSignedUrls` calls (verified 2026-09-07)                                                                            |
| 32  | Most data fetched client-side after hydration despite SSR — skeletons on first paint                                                                                  | Med      | **partially fixed** (2026-09-08) — landing stats/projects/activity, public profiles, and project titles are prefetched through route loaders and dehydrated React Query state; authenticated routes remain client-gated by design (`/_authenticated` uses `ssr: false`)                                                                  |
...
| 36  | Entry client chunk over bundle budget — every route component shipped eagerly (974.5KB raw vs 900KB budget)                                                           | High     | **fixed** (2026-09-18) — all 23 routes converted to the TanStack lazy pattern (`lazyRouteComponent` + `-page.tsx` dash modules, shared `-u.$handle-data.ts` for eager-loader/lazy-page data); entry chunk 974.5KB → ~727–822KB depending on build, all 509–512 chunks within budgets; conventions documented in `TETHYR_ARCHITECTURE.md` |
| 37  | 168 dead exports tracked in the unused-exports baseline — dead code accumulating under a growing ignore-list                                                          | Medium   | **fixed** (2026-09-18) — 48 fully-dead declarations deleted (incl. the orphaned `space-settings-dialog` component), 120 de-exported to private in-file helpers, cascade-cleaned via lint; baseline now **zero** and enforced (`check:unused` fails on any new unused export); policy in `TETHYR_ARCHITECTURE.md`                         |
| 38  | Landing main-thread blocking (TBT ~490ms) and public-surface a11y/SEO gaps (heading order, skip-link target, prohibited ARIA, generic link text, small touch targets) | Medium   | **fixed** (2026-09-18) — `SectionReveal` rewritten from framer-motion to IntersectionObserver + CSS (content visible by default for SSR/no-JS), sonner Toaster mounts on browser idle: landing TBT 490→280ms, Performance 74→84; Lighthouse (desktop) a11y/SEO 100 on landing + project page, 98/100 on /skills                          |

## 16. Testing

Strength: 71 test files (hooks, components, lib).

| #   | Issue                                                                                                                                                                                 | Severity | Status                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 35  | Critical paths untested — messages, sessions, challenges (hooks), teams, templates, library, settings, notifications hook, connections hook, follow, skill match, reputation, credits | Medium   | open — messages/sessions/challenges (2026-09-05), follow + credits (2026-09-05), and library/notifications/connections/skill-match (2026-09-07: `use-library.test.tsx` 12 + `use-notifications.test.tsx` 9 + `use-connections.test.tsx` 7 + `skill-match.test.ts` 14) now covered; teams + templates have baselines; settings is covered at the route/component level (`-settings.test.tsx`, `github-connect.test.tsx`) |
...
| 3   | Dual rendering path on project page (blocks + legacy) — maintenance debt          | Medium   | **fixed** (2026-09-07) — workspace is the only project render; block takeover + orphan tab bar removed (see section 2, #3)                                                                                                                                                                                       |
| 4   | N+1 signed URL generation for contributor avatars                                 | Low-Med  | **fixed** (2026-09-05) — `projects.$id.tsx` now uses one batched `createSignedUrls` for contributor avatars instead of a per-avatar loop; the remaining project-media cover loops in `explore.tsx` + `use-current-user.ts` were also batched into single `createSignedUrls` calls (verified 2026-09-07)          |
...
| 7   | Skill matching is client-side — won't scale                                       | Low      | **fixed** (2026-09-08) — project and creator recommendations now use authenticated Postgres RPCs with bounded results and indexed skill joins                                                                                                                                                                    |
...
| 11  | No email verification enforcement visible                                         | Low-Med  | open — UX shipped (2026-09-07: unverified banner + login resend); enforcement itself stays a product/Supabase-config decision (needs demo/seed exemption path)                                                                                                                                                   |
...
- **N+1 signed-URL loops for covers** (previously noted on `src/hooks/use-current-user.ts` and `src/routes/_authenticated/explore.tsx`): both were batched into a single `createSignedUrls` call — no per-item cover loops remain (verified 2026-09-07).
