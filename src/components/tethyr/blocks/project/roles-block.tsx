...
function ProjectRolesBlock({ config, context }: BlockProps) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: d } = await supabase
        ...
        .select("id,title,description,skills,is_filled")
        .eq("project_id", projectId)
        .order("created_at", { ascending: false });
      return (d ?? []) as unknown as RoleRow[];
    },
    enabled: !!projectId,
  });
  ...
  if (!data?.length) {
    if (context.isEditing)
      return <BlockEmptyState label="Open Roles" detail="Open roles will appear here." />;
    return null;
  }
    ...
    <div>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {data.map((r) => (
          <div
            key={r.id}
            className={`rounded-lg border p-3 ${r.is_filled ? "border-border/40 opacity-60" : "border-border bg-surface"}`}
          >
