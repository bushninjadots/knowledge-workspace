...
function ProjectDiscussionsBlock({ config, context }: BlockProps) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: d } = await supabase
        ...
        .limit(20);
      return (d ?? []) as unknown as DiscussionRow[];
    },
    enabled: !!projectId,
  });
  ...
  if (!data?.length) {
    if (context.isEditing)
      return <BlockEmptyState label="Discussions" detail="Discussions will appear here." />;
    return null;
  }
  return (
    <div>
        ...
        {data.slice(0, 10).map((d) => (
          <div key={d.id} className="rounded-lg border border-border bg-surface p-3">
            <div className="flex items-center gap-2 text-[11px] text-muted-foreground mb-1">
              ...
              {d.profiles?.display_name ?? d.profiles?.handle ?? "Someone"}
