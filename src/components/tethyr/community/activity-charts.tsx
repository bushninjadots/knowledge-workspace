      ...
      <div className="mb-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground">
            {label === "joins" ? "New Members" : "New Posts"}
          </span>
        </div>
          ...
          >
            <div
              ...
              style={{
                ...
                background:
                  bar.value > 0 ? color : `color-mix(in oklch, ${color} 15%, transparent)`,
              }}
            />
          </div>
        ))}
    ...
    <section className="mb-6 rounded-xl bg-surface-elevated/30 p-4">
      ...
      {isEmpty ? (
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:gap-8">
          <MiniBarChart data={activity} color="var(--trust)" label="joins" icon={UserPlus} />
          <div className="hidden sm:block w-px bg-border/40" />
          <MiniBarChart data={activity} color="var(--primary)" label="posts" icon={MessageSquare} />
        </div>
      )}
    </section>
  );
