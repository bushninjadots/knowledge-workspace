              ...
              {group.items.map((item) => {
                ...
                if (item.href) {
                  return (
                    ...
                    >
                      {isActive && (
                        <span className="absolute left-0 top-1/2 h-5 -translate-y-1/2 rounded-full bg-[var(--user-accent,var(--primary))]" style={{ width: 3 }} />
                      )}
                      <Icon className={cn("h-4 w-4 shrink-0", isActive && "text-[var(--user-accent,var(--primary))]")} />
