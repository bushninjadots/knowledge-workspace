    ...
    >
      <div className="mx-auto grid max-w-md grid-cols-5 gap-1">
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.to}
              to={item.to}
              className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-md text-[10px] transition-lift ${
                active
        ...
        <button
          type="button"
          onClick={onOpenMore}
          className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-md text-[10px] text-muted-foreground transition-lift hover:text-foreground"
