...
import { Edit3, Palette, Plus, Send, X, Bookmark, GalleryHorizontalEnd } from "lucide-react";
...
import { useSaveAsTemplate, useApplyTemplate, useMyTemplates } from "@/hooks/use-templates";
import { ThemePicker } from "@/components/tethyr/page/theme-picker";
import { useCurrentThemeInfo } from "@/hooks/use-theme-catalog";
    ...
    <>
      <div className="mb-6 border-b border-border/30 pb-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-muted-foreground/30 mx-1">·</span>
          <Button variant="ghost" size="sm" className="h-7 gap-1 text-[11px]" onClick={stopEditing}>Done</Button>
          {isPublished ? (
...
function BlockPickerPanel({ onAdd, onClose }: BlockPickerPanelProps) {
  ...
  return (
    <div className="relative mb-4 rounded-xl border border-card-border bg-surface-elevated p-4">
      ...
      <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Add a block</h3>
