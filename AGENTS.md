...
- Start command: `bash scripts/base44-start.sh` (recorded in `.base44/environment.json`).
  It runs `npx supabase start` (brings up the full local Supabase stack and applies all 188 migrations + the demo seed on first init), captures the live API keys from `supabase status`, writes them to the gitignored `.env.supabase-runtime`, then runs `docker compose -f docker-compose.base44.yml up -d --build`.
...
- **Endpoint precedence is load-bearing and lives in a generated file.** `resolveSupabaseEnv()` in `src/integrations/supabase/client.ts` must prefer `SUPABASE_URL` / `SUPABASE_PUBLISHABLE_KEY` when `typeof window === "undefined"`, and the `VITE_` values in the browser. In a container, `VITE_SUPABASE_URL` (e.g. `http://127.0.0.1:54321`) points at the _browser's_ localhost, which SSR cannot reach — if the `VITE_` value wins on the server, every server-side Supabase read fails silently and data-dependent routes render loading states that then disagree with the hydrated client.
- `src/integrations/supabase/client.ts` carries Lovable's `// This file is automatically generated. Do not edit it directly.` header. It is the only home for that precedence (141 modules import the client), so a Lovable regeneration can silently revert it. `src/integrations/supabase/client.test.ts` fails loudly if it does — do not delete that test, and re-apply the precedence if the file is regenerated.
...
- **PostgREST aggregate selects are environment-dependent — `PGRST123`.** The bundled local PostgREST runs with `db_aggregate_functions_enabled = false`, so an aggregate select such as `.select("project_id, count()")` returns **400** locally while working against the hosted stack. Typecheck, lint, and unit tests are all blind to it; `scripts/qa-audit.mjs` is what catches it, which is why it records console/HTTP errors per page. Group in Postgres instead — a small SQL function, `SECURITY INVOKER` when the count must respect the caller's RLS (`20260922103000_explore_count_rpcs.sql`), `SECURITY DEFINER` only when the number is deliberately global (`20260922102000_community_daily_activity_rpc.sql`). The header-count form `.select("id", { count: "exact", head: true })` works on every stack and needs no function.
- **`anon` gets EXECUTE on every new function unless a migration revokes it.** Supabase's default privileges for `postgres` and `supabase_admin` grant EXECUTE on functions in `public` to `anon`, `authenticated`, and `service_role`, and `REVOKE ... FROM PUBLIC` does not remove those explicit grants — so `REVOKE ALL ON FUNCTION ... FROM PUBLIC` leaves a function callable by a signed-out visitor. `20260922105000_least_privilege_anon_execute.sql` fixed the default for `postgres` and revoked the remaining `SECURITY DEFINER` gaps; `supabase/tests/anon_execute_grants.sql` pins the exact allowlist, and `supabase test db` runs it in CI. This class has shipped four times, once when a full-schema dump migration re-granted `anon` to a function a hardening migration had just revoked.
- **Run the production build with `npm run build:prod` + `npm run serve:prod`, not `npm run build`.** The deployed artifact is a single-file fetch bundle: the sandbox build sets `LOVABLE_NITRO_PRESET=lovable-fetch-bundle`, which configures `inlineDynamicImports: true` and writes `dist/server/index.mjs` (`dist/` is gitignored). Building with the fallback presets instead — the default `cloudflare-module` under `wrangler dev`, or `NITRO_PRESET=node-server` under node — produces a chunk-split SSR bundle that dies at import with `TypeError: __exportAll is not a function`: rolldown hoists that helper into a sibling chunk (`_ssr/server-<hash>.mjs`) that sits in a require cycle with the chunk consuming it. That is a bundler artifact of a preset nothing deploys, not an app bug, so do not chase it — `scripts/serve-prod-build.mjs` runs the real bundle over HTTP (static assets from `dist/client`, everything else through `fetch`), which is how a production-only behaviour (CSP header mode, `NODE_ENV` branches, prod bundle size) gets checked locally: `node scripts/qa-csp.mjs --base http://127.0.0.1:8790 --require-enforced` and `node scripts/qa-audit.mjs --base http://127.0.0.1:8790`. It needs `SUPABASE_URL`/`SUPABASE_PUBLISHABLE_KEY`/`SUPABASE_SERVICE_ROLE_KEY` in the environment (the `VITE_` pair is baked in at build time). Two checks in `scripts/qa-workflows.mjs` import app _source_ modules by URL, so they only pass against the dev server.
...
- **CSP nonces are per request and React hides the attribute.** The middleware in `src/start.ts` generates the nonce, sets `Content-Security-Policy` (report-only in dev, where Vite's inline scripts have no nonce), and passes it through the request context; `src/router.tsx` hands it to `router.options.ssr.nonce`, and `src/routes/__root.tsx` stamps the theme bootstrap by hand. On the client the router reads it back from `<meta property="csp-nonce">`. Two traps: React renders the nonce through the **IDL property**, so `el.getAttribute("nonce")` is `""` after hydration — inspect `el.nonce` instead (`scripts/qa-csp.mjs` does); and the nonce-less fallback policy in `addSecurityHeaders` must stay strict, so any page served outside the middleware (the static error page, `/sitemap.xml`, `/robots.txt`) may not use inline scripts or inline event handlers.

### Verifying it works

- `curl -H "Host: <any>" http://localhost:3000/` returns 200 with `<title>Build together, get known for what you make</title>`.
- For anything only true of a production build (CSP header mode, `NODE_ENV` branches, bundle size): `npm run build:prod`, then `npm run serve:prod` with the Supabase env vars set, then point the harnesses at it — `node scripts/qa-audit.mjs --base http://127.0.0.1:8790` and `node scripts/qa-csp.mjs --base http://127.0.0.1:8790 --require-enforced`. This is the deployed artifact shape, and it is the only way to see the enforced nonce policy (`script-src 'self' 'nonce-…'`) rather than the dev server's report-only one.
- The SSR HTML reaches Supabase and carries real seeded data (project titles such as "Bloom"/"Reverb"/"Atlas" appear in the serialized React Query payload). The landing sections themselves still paint their loading states on first paint, because the server render happens before the route's `prefetchQuery` resolves; the client fills them in immediately after hydration.
