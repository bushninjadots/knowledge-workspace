...
- **The local production build is not runnable; only the dev server is.** `npm run build` succeeds and produces a Cloudflare (`cloudflare-module`) or `node-server` bundle, but running either (`npx wrangler dev -c .output/server/wrangler.json`, or `node .output/server/index.mjs` with `NITRO_PRESET=node-server`) fails with `TypeError: __exportAll is not a function` — Nitro's `_libs` chunk splitting hoists an esbuild helper that some chunks then reference without defining it. `.output` is gitignored and the platform serves `npm run dev`, so this has never been exercised. It does mean **a change that only manifests in a production build (CSP header mode, `NODE_ENV` branches) cannot be verified end to end locally** — verify it structurally instead (unit tests plus the dev-server report-only policy) and say so, rather than assuming `npm run build` proves it.
