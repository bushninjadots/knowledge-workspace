-- Project return loop: durable evidence, direction, return context, and meaningful recognition.
...
ALTER TABLE public.projects
  DROP CONSTRAINT IF EXISTS projects_season_check;
ALTER TABLE public.projects
...
-- A project can be watched without changing the existing person-follow graph.
...
-- Last-seen timestamps make return context personal and cross-device.
...
-- Recognition attaches to evidence, not to a generic popularity score.
...
DO $$ BEGIN
  CREATE POLICY "Project recognitions are readable"
    ...
    USING (
      ...
      OR EXISTS (
        SELECT 1 FROM public.projects p
        WHERE p.id = project_recognitions.project_id
          AND p.visibility = 'public'
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- Contributors can add evidence entries; project owners retain deletion control.
DO $$ BEGIN
...
-- Keep the public shelf intentionally small and owner-curated. Each item is a
-- JSON object: { project_id, activity_id, title, note, url, kind }.
...
-- Evidence contributions and recognition feed the existing evidence-based
-- reputation system without creating a popularity score.
...
FOR EACH ROW
EXECUTE FUNCTION public.record_project_evidence_contribution();
...
FOR EACH ROW
EXECUTE FUNCTION public.record_project_recognition();
